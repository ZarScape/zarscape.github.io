    ...
    <div className="flex items-center gap-2">
      {[0, 1, 2].map((index) => (
        <motion.div
          ...
          className="h-4 w-4 rounded-full border border-neutral-300/60 bg-gradient-to-b from-cyan-200 to-cyan-400 shadow-[0_0_20px_rgba(34,211,238,0.28)]"
        />
      ))}
    </div>
  );
  ...
  return (
    <div className="flex items-center">
      ...
      <motion.div
        ...
        className="-translate-x-2 h-4 w-4 rounded-full bg-neutral-200 shadow-md dark:bg-neutral-500"
      />
      <motion.div
        initial={{ x: 0 }}
        animate={{ x: [0, 20, 0] }}
        transition={transition(0.8)}
        className="-translate-x-4 h-4 w-4 rounded-full bg-neutral-200 shadow-md dark:bg-neutral-500"
      />
    </div>
  );
}
...
export function LoaderFour({ text = 'Loading...' }) {
  return (
...
export function LoaderFive({ text }) {
  return (
